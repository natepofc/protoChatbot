...
def find_audio_devices():
    ...
    # Get default devices first (works on Mac)
    try:
...
def play_audio(filename):
    ...
    try:
        ...
        if output_index is None:
            print("⚠️ No speaker found, skipping playback.")
            wave_file.close()
...
def record_audio(filename="input.wav", threshold=2400, silence_duration=0.6):
    ...
    Voice-activated audio recorder:
      ...
      - Can also be stopped by pressing ENTER key
    """
    audio_interface = pyaudio.PyAudio()
    ...
    try:
        ...
        print("   This might be a microphone permissions issue on Mac.")
        print("   Check System Preferences > Security & Privacy > Microphone")
        audio_interface.terminate()
        return None

    # Start the stream explicitly and give it a moment to initialize
    stream.start_stream()
    time.sleep(0.1)  # Small delay to ensure stream is ready

    print("🎤 Recording... (Press ENTER to stop)")
    print("   (Monitoring audio levels - speak clearly)")
    ...
    try:
        # Start recording immediately - no voice activation needed
        ...
        while True:
            ...
            try:
                ...
                # Show audio level every 20 chunks (about every 0.5 seconds)
                if chunk_count % 20 == 0:
                    print(f"   Recording... | RMS: {rms:6d} | Max: {max_rms:6d}", end='\r')
            except Exception as e:
    ...
    if chunk_count > 0:
        ...
        print(f"   - Samples with audio (>500): {samples_with_audio}/{chunk_count}")
        
        if max_rms < 500:
            print(f"\n   ⚠️  WARNING: Very low audio levels detected!")
            print(f"   This might indicate:")
            print(f"   1. Microphone permissions not granted")
            print(f"   2. Microphone is muted or volume is very low")
            print(f"   3. No sound was captured during recording")
        elif samples_with_audio < chunk_count * 0.1:
            print(f"\n   ⚠️  WARNING: Very little audio detected!")
            print(f"   Only {samples_with_audio} out of {chunk_count} samples had significant audio.")
        else:
...
def main():
    ...
    speak_text("I'm ready. Ask me a question.", color=(0, 255, 0))

    # Initialize armed state - always ready to listen (no button needed)
    is_armed = True
    ...
    try:
        while True:
            ...
            # Play back the recorded audio before transcribing
